...
(defn try-create-topic
  ...
  (let [topic-auto-create (get-in state [:conf :topic-auto-create])]
    ...
    (if topic-auto-create
      ...
      (throw (RuntimeException. (str "Could not create topic " topic " as topic-auto-create is disabled, and no topic found"))))))
  ...
  (loop [retry 0]
    (if (and (-> state :conf :batch-fail-message-over-limit) (>= (count bts) ^long (-> state :conf :batch-byte-limit)))
      ...
      (if (first
        ...
        ;;checking for boolean false here, apparently some code generated by clojure async in some versions and situations
        ...
        (if (> retry 3)
          (throw (RuntimeException. (str "No healthy brokers available for topic " topic " after 3 metadata update retries")))
          (do
